...
class ExcelPreview extends React.Component {
  ...
  getStaticName = (e, pageName) => {
    if (pageName === "Preventifs")
      return this.preventifColumnMapping[e]
        ? this.preventifColumnMapping[e]
        : e;
    if (pageName === "Correctifs")
      return this.correctifColumnMapping[e]
        ? this.correctifColumnMapping[e]
        : e;
    if (pageName === "données fixe Equipements")
      return this.equipementColumnMapping[e]
        ? this.equipementColumnMapping[e]
        : e;
    if (pageName === "etat de stock")
      return this.stockColumnMapping[e] ? this.stockColumnMapping[e] : e;
    if (
      this.props.dataType === "_doc_data" ||
      this.props.dataType === "_qsse_data" ||
      this.props.dataType === "_schema_data"
    )
      return this.documentationColumnMapping[e]
        ? this.documentationColumnMapping[e]
        : e;
  ...
  render() {
    return (
      <>
        <Row className="">
          <span className="_excel_title">
            {GRAPHICS_PAGE_FR.EXCEL_PREVIEW_TITLE}
          </span>
        </Row>
        <br></br>
        <br></br>
        <Spin
          spinning={this.state.isProcessingData}
          tip={this.state.processingMessage}
          size="large"
        >
          <ReactExcel
          ...
          <Input
            ...
            value={this.selectedEq}
            onChange={(e) => this.setState({ selectedEq: e.target.value })}
          ></Input>
          <Button
            style={{
              ...
              if (Object.keys(this.state.currentSheet).length > 0) {
                ...
                const reqPutItems =
                  pageName === "Preventifs"
                    ...
                    : pageName === "Correctifs"
                    ? reqPostCorrectifs
                    : pageName === "données fixe Equipements"
                    ? reqPostEquipements
                    : pageName === "etat de stock"
                    ? reqPostStock
                    : this.props.dataType === "_doc_data"
                    ? reqPostDocs
                    : this.props.dataType === "_qsse_data"
                    ...
                    : this.props.dataType === "_schema_data"
                    ? reqPostSchema
                    : this.props.dataType === "_mesures_data"
                    ? reqPostMesure
                    : null;
                ...
                if (this.props.dataType !== "_mesures_data") {
                  ...
                  let res = {
                    ...
                    seuil:
                      this.state.currentSheet[pageName][1][
                        columnsNames.indexOf("Seuil")
                      ],
                    values: [],
                  };
                  this.state.currentSheet[pageName].forEach((elem, ind) => {
