...
router.post("/api/v1.0/signup", (req, res) => {
  try {
    ...
    !User_serializer.is_valide({ raise_exception: false }) &&
    ...
    USER.findOne(
      ...
      (err, existingUser) => {
        if (existingUser) {
          if (!existingUser.isActive)
            setHeaders({ res, status: 450 }).then(() =>
              res.end(
                _JSON2STR({
                  err_number: 12,
                  demande_state: ERROR_MESSAGES_EN[12],
                })
              )
            );
          else
            setHeaders({ res, status: 450 }).then(() =>
              res.end(
                _JSON2STR({
                  err_number: 13,
                  demande_state: ERROR_MESSAGES_EN[13],
                })
              )
            );
        } else {
          ...
          UserModel.password =
            User_serializer.validated_data["crypted_password"];
            ...
            err &&
              setHeaders({ res, status: 450 }).then(() =>
                res.end(
                  _JSON2STR({
                    err_number: 14,
                    demande_state: ERROR_MESSAGES_EN[14],
                  })
                )
              );
            !err &&
              setHeaders({ res, status: 200 }).then(() =>
                res.end(_JSON2STR({ isActive: false, idUser: reply }))
              );
          });
        }
      }
    ...
    console.log(e);
    setHeaders({ res, status: 500 }).then(() =>
...
router.post("/api/v1.0/signin", (req, res) => {
  try {
    ...
    !Login_serializer.is_valide({ raise_exception: false }) &&
      setHeaders({ res, status: 450 }).then(() =>
        res.end(
          _JSON2STR({ err_number: 11, demande_state: ERROR_MESSAGES_EN[11] })
        )
    ...
    USER.findOne(
      ...
      (err, existingUser) => {
        if (
          existingUser &&
          Login_serializer.validatePassword(existingUser.password)
        ) {
          if (!existingUser.isActive)
            setHeaders({ res, status: 450 }).then(() =>
              res.end(
                _JSON2STR({
                  err_number: 16,
                  demande_state: ERROR_MESSAGES_EN[16],
                })
              )
            );
          else {
            ...
            setHeaders({ res, status: 200 }).then(() =>
              res.end(
                _JSON2STR({
                  ...
                  idUser: existingUser._id,
                  demande_state: SUCCESS_MESSAGES_EN[20],
                })
              )
            );
          }
          ...
          setHeaders({ res, status: 450 }).then(() =>
            res.end(
              _JSON2STR({
                err_number: 15,
                demande_state: ERROR_MESSAGES_EN[15],
              })
            )
          );
      }
